...
/* 	Issues:
        ...
        2)	would got wrong answer when b = 0 and a >= mod, so check it
   carefully!
*/
...
*/
LL getInv(LL a, int mod = Math::MOD) { return qp(a, mod - 2, mod); }
...
/*	Caculate number of permutations:
        number of ways to select b balls from a balls (pairwise distinct) to
   form an array.

        Issues:
        1)	make sure that you have initialized the array first
   (Math::init(size, mod)); 2)	make sure that b <= a <= size;
